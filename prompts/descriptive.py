...
logging.basicConfig(filename='/home/shivangi/data-to-story/benchmarking/qgen_logger_predictive2.log', level=logging.INFO, filemode='a', format='%(asctime)s - %(levelname)s - %(message)s')
...
Return a single JSON array named `questions`, not multiple arrays."""
    ...
    def generate(self, database_name):
        ...
        try:
            ...
            prompt = self.get_descriptive_prompt(schema)
...
if __name__ == "__main__":
    ...
    file_path = "./question_generation_benchmarking/question_jsons/descriptive_questions.json"

    # Step 4: Write the formatted JSON to a file
    with open(file_path, "w") as file:
        json.dump(ques, file, indent=4)  # Write JSON properly

    print(f"JSON successfully written to {file_path}")
